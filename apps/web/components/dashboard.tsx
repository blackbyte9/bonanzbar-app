...
type BarEvent = { id: string; title: string; description: string | null; location: string | null; bandInfo: string | null; bandHomepageUrl: string | null; bandImageUrls: string[]; ticketUrl: string | null; youtubeUrl: string | null; sourceUrl: string | null; startsAt: string; endsAt: string | null; status: "DRAFT" | "PUBLISHED" | "CANCELLED"; duties: EventDuty[] };
    ...
    return (
      <main className="auth-shell">
        ...
        <section className="login-card">
          ...
          <button type="button" className="secondary compact-button public-program-button" onClick={() => setShowPublicProgram(true)}>Öffentliches Programm ansehen</button>
...
function HomepageProgramImport({ request, setMessage, onUpdated }: { request: (path: string, init?: RequestInit) => Promise<unknown>; setMessage: (value: string) => void; onUpdated: () => Promise<void> }) {
  const [importing, setImporting] = useState(false);
  const importProgram = async () => {
    setImporting(true);
    try {
      const result = await request("/api/events/import-jimdo", { method: "POST" }) as { created: number; existing: number };
      setMessage(`${result.created} Termin(e) als Entwurf importiert${result.existing > 0 ? `; ${result.existing} bereits importierte Termin(e) wurden nicht überschrieben` : ""}.`);
      await onUpdated();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Das Homepage-Programm konnte nicht importiert werden.");
    } finally {
      setImporting(false);
    }
  };
  return <section className="content"><section className="panel"><h1>Homepage-Programm importieren</h1><p className="muted">Importiert neue Termine von <a href="https://bonanzbar.jimdofree.com/" target="_blank" rel="noreferrer">bonanzbar.jimdofree.com ↗</a> als Entwürfe. Ticket-Links und vorhandene Poster werden übernommen; bereits importierte Termine und spätere Änderungen bleiben unverändert.</p><button type="button" className="primary" onClick={() => void importProgram()} disabled={importing}>{importing ? "Importiere …" : "Programm von Homepage importieren"}</button></section></section>;
}
  ...
  return <section className="content two-column">
    <section className="panel">
      ...
      <form onSubmit={(event) => void create(event)}>
        ...
        <div className="approval-fields"><Field name="startsAt" label="Beginn" type="datetime-local" initial={new Date(Date.now() + 6048e5).toISOString().slice(0, 16)} /><Field name="endsAt" label="Ende (optional)" type="datetime-local" required={false} /><Field name="location" label="Ort (optional)" required={false} /></div>
        <label>Beschreibung<textarea name="description" rows={3} /></label>
        <label>Bandinfos<textarea name="bandInfo" rows={3} /></label>
        <div className="approval-fields"><Field name="bandHomepageUrl" label="Band-Homepage (URL)" type="url" required={false} /><Field name="ticketUrl" label="Ticket-Link (URL)" type="url" required={false} /><Field name="youtubeUrl" label="YouTube-Link (URL)" type="url" required={false} /></div>
        <label>Bandbild-URLs (maximal 4, eine URL je Zeile)<textarea name="bandImageUrls" rows={3} /></label>
        <label>Status<select name="status" defaultValue="DRAFT"><option value="DRAFT">Entwurf</option><option value="PUBLISHED">Veröffentlicht</option></select></label>
        <label>Dienste und Sollbesetzung<textarea name="duties" rows={5} defaultValue={"Theke;2\nEintritt;1\nJoker;1"} /><small>Ein Dienst je Zeile im Format „Bezeichnung; Anzahl“.</small></label>
        <button className="primary">Veranstaltung speichern</button>
      </form>
    </section>
    <section className="panel">
      <h2>Veranstaltungen verwalten</h2>
      {events.length === 0 ? <p className="muted">Noch keine Veranstaltung geplant.</p> : <div className="event-list">{events.map((event) => <article className="event-card" key={event.id}>{editingId === event.id ? <form className="event-edit-form" onSubmit={(submitEvent) => void updateEvent(submitEvent, event)}>
        <div className="edit-heading"><b>{event.title} bearbeiten</b><button type="button" className="secondary compact-button" onClick={() => setEditingId(null)}>Abbrechen</button></div>
        <div className="approval-fields"><Field name="title" label="Titel" initial={event.title} /><Field name="startsAt" label="Beginn" type="datetime-local" initial={dateTimeInput(event.startsAt)} /><Field name="endsAt" label="Ende (optional)" type="datetime-local" initial={event.endsAt ? dateTimeInput(event.endsAt) : undefined} required={false} /><Field name="location" label="Ort (optional)" initial={event.location ?? undefined} required={false} /></div>
        <label>Beschreibung<textarea name="description" rows={3} defaultValue={event.description ?? ""} /></label>
        <label>Bandinfos<textarea name="bandInfo" rows={3} defaultValue={event.bandInfo ?? ""} /></label>
        <div className="approval-fields"><Field name="bandHomepageUrl" label="Band-Homepage (URL)" type="url" initial={event.bandHomepageUrl ?? undefined} required={false} /><Field name="ticketUrl" label="Ticket-Link (URL)" type="url" initial={event.ticketUrl ?? undefined} required={false} /><Field name="youtubeUrl" label="YouTube-Link (URL)" type="url" initial={event.youtubeUrl ?? undefined} required={false} /></div>
        <label>Bandbild-URLs (maximal 4, eine URL je Zeile)<textarea name="bandImageUrls" rows={3} defaultValue={event.bandImageUrls.join("\n")} /></label>
